                     ...
                     usage:(GLenum)usage;
{
    ...
    if(self != nil)
    {
         ...
         */
        // STEP 3 数据拷贝到缓存对象
        glBufferData(GL_ARRAY_BUFFER,   // Initialize buffer contents:初始化缓冲内容
                     _bufferSizeBytes,  // Number of bytes to copy:要复制的字节数
                     dataPtr,           // Address of bytes to copy:要复制的字节的地址
                     usage);            // Hint: cache in GPU memory:GPU存储器中的缓存
    }
                   ...
                   shouldEnable:(BOOL)shouldEnable
{
    ...
    /*
     ...
     ptr:偏移量
     */
    glVertexAttribPointer(index,            // 参数指定顶点属性位置
                          count,            // 指定顶点属性大小
                          GL_FLOAT,         // 指定数据类型
                          GL_FALSE,         // 数据被标准化
                          (int)self.stride, // 步长
