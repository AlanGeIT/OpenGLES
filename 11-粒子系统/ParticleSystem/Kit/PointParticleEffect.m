...
@property (nonatomic, strong, readwrite) VertexAttribArrayBuffer    *particleAttributeBuffer;   //顶点属性数组缓冲区
@property (nonatomic, assign, readonly)  NSUInteger                 numberOfParticles;          //粒子个数
@property (nonatomic, strong, readonly)  NSMutableData              *particleAttributesData;    //粒子属性数据
@property (nonatomic, assign, readwrite) BOOL                       particleDataWasUpdated;     //是否更新粒子数据
...
-(id)init
{
    ...
    if (self != nil) {
        ...
        // 2.坐标变换的信息用于GLKit渲染效果。GLKEffectPropertyTransform类定义的属性进行渲染时的效果提供的坐标变换。
        ...
        */
        ...
        if(self.particleDataWasUpdated)
        {
            ...
            if(self.particleAttributeBuffer == nil && [self.particleAttributesData length] > 0) {
                ...
                // 开辟空间
                self.particleAttributeBuffer = [[VertexAttribArrayBuffer alloc] initWithAttribStride:size
                ...
                // 重新开辟空间
                [self.particleAttributeBuffer reinitWithAttribStride:size
                                                    numberOfVertices:count
                                                               bytes:[self.particleAttributesData bytes]];
            }
        ...
        // 准备顶点数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib: CCParticleEmissionPosition
                                          numberOfCoordinates: 3
                                                 attribOffset: offsetof(CCParticleAttributes, emissionPosition)
                                                 shouldEnable: YES];
        
        // 准备粒子发射速度数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib: CCParticleEmissionVelocity
                                          numberOfCoordinates: 3
                                                 attribOffset: offsetof(CCParticleAttributes, emissionVelocity)
                                                 shouldEnable: YES];
        
        // 准备重力数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib: CCParticleEmissionForce
                                          numberOfCoordinates: 3
                                                 attribOffset: offsetof(CCParticleAttributes, emissionForce)
                                                 shouldEnable: YES];
        
        // 准备粒子size数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib: CCParticleSize
                                          numberOfCoordinates: 2
                                                 attribOffset: offsetof(CCParticleAttributes, size)
                                                 shouldEnable: YES];
        
        // 准备粒子的持续时间和渐隐时间数据
        [self.particleAttributeBuffer prepareToDrawWithAttrib: CCParticleEmissionTimeAndLife
                                          numberOfCoordinates: 2
                                                 attribOffset: offsetof(CCParticleAttributes, emissionTimeAndLife)
                                                 shouldEnable: YES];
...
- (BOOL)loadShaders
{
    ...
    // 创建并编译 vertex shader.
    vertShaderPathname = [[NSBundle mainBundle] pathForResource: @"PointParticleShader" ofType:@"vsh"];
    ...
    fragShaderPathname = [[NSBundle mainBundle] pathForResource: @"PointParticleShader" ofType:@"fsh"];
     ...
     */
    // 位置
    glBindAttribLocation(program, CCParticleEmissionPosition, "a_emissionPosition");
    // 速度
