...
#import "ParticleSystemVC.h"
#import "VertexAttribArrayBuffer.h"
#import "PointParticleEffect.h"

@interface ParticleSystemVC ()

@property (nonatomic, strong) EAGLContext           *mContext;              // 上下文
@property (nonatomic, strong) PointParticleEffect   *particleEffect;        // 管理并且绘制所有的粒子对象
@property (nonatomic, assign) NSTimeInterval        autoSpawnDelta;         // 动画时间
@property (nonatomic, assign) NSTimeInterval        lastSpawnTime;          // 上一次更新时间
@property (nonatomic, assign) NSInteger             currentEmitterIndex;
@property (nonatomic, strong) NSArray               *emitterBlocks;
@property (nonatomic, strong) GLKTextureInfo        *ballParticleTexture;   //粒子纹理对象

@end

@implementation ParticleSystemVC
...
-(void)viewDidLoad
{
    ...
    // 粒子对象
    self.particleEffect = [[PointParticleEffect alloc]init];
...
- (void)preparePointOfViewWithAspectRatio:(GLfloat)aspectRatio
{
     ...
     */
    self.particleEffect.transform.modelviewMatrix = GLKMatrix4MakeLookAt(0.0, 0.0, 1.0,   // Eye position
